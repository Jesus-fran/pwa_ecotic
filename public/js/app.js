...
function crearMensajeHTML(mensaje, personaje, tel, lat, lng, foto) {
    // console.log(mensaje, personaje, lat, lng);
    var content = `
    <div class="col-md-6 col-lg-6 col-sm-12 col-xs-12"> <li class="animated fadeIn fast"
        data-tipo="mensaje">
        <div class="bubble-container">
            <div class="bubble">
                <h6 class="fw-bold">~${ personaje } - ${tel}</h6>
                <br/>
                ${ mensaje }
                `;
    if (foto) {
        content += `
                <br>
                <img class="foto-mensaje" src="${ foto }"><div>
        `;
    }
    ...
    if (mensaje == "Ningun mensaje por ahora") {
        content = "<div class='col-12 text-center' id='mensaje1'>No hay mensajes :c </div>";
    } else {
        $('#mensaje1').remove();
    }
...
function logIn(ingreso) {
    ...
    if (ingreso) {
        ...
        titulo.text('Bienvenido a EcoTic');
...
postBtn.on('click', function() {
    ...
    var usuario = $("#name_user").val();
    var tel = $("#tel_user").val();
    console.log(usuario);
    crearMensajeHTML(mensaje, usuario, tel, lat, lng, foto);
...
function getMensajes() {
    ...
    fetch('api')
        ...
        .then(posts => {
            ...
            posts.forEach(post =>
                crearMensajeHTML(post.mensaje, post.user, post.tel, post.lat, post.lng, post.foto));
        });
...
btnLocation.on('click', () => {
    $.mdtoast('Cargando el mapa, espere porfavor...', {
        interaction: true,
